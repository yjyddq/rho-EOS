...
LOG_FILE="./logs/run_llada_baseline_$(date +%Y%m%d_%H%M%S).log"
...
for task in "${TASKS[@]}"; do
    for length in "${LENGTHS[@]}"; do
        ...
        EOS_LOG_PATH="./results/baseline_eos_log/${task}_${length}_eos_stats.log"
        ...
        accelerate launch --config_file "$ACCEL_CONFIG" evaluation_script.py \
            ...
            --tasks "${task}" \
            --batch_size 8 \
            --model_args "pretrained=${MODEL_PATH},assistant_prefix=<reasoning>,eos_log_file=${EOS_LOG_PATH} " \
